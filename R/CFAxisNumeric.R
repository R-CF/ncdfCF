  ...
  public = list(
    ...
    append = function(from) {
      if (super$can_append(from) && .c_is_monotonic(private$.values, from$values)) {
        bnds <- if (is.null(private$.bounds) || is.null(from$bounds)) NULL
                else cbind(private$.bounds, from$bounds)
        makeAxis(self$name, axis$orientation, c(private$values, from$values), bnds, self$attributes)
      } else
