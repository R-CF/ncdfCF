...
table. Objects with a `standard_name` attribute can be identified with the
`CFDataset$objects_by_standard_name()` method, either for presence of the attribute
or using a specific standard name.
...
Upon writing, the deprecated `gregorian` calendar is replaced by the equivalent
`standard` calendar.
...
Climatological statistics can be generated using the `CFVariable` or `CFArray`
`summarise()` method and written to file, including its bounds.
