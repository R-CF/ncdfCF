...
![Figure 2: The ncdfCF data model](ncdfCF-datamodel-0.4.png){width="680"} This
...
Fully implemented. Axes of length 1 are automatically encoded as a
scalar coordinate variable, thus avoiding the use of a dimension.

##### 2.5. Variables

The `valid_range` attribute is determined when a `CFArray` is created or
its values modified. The attribute is also attached to all axes and their
boundaries.
...
Upon writing, the deprecated `gregorian` calendar is replaced by the
equivalent `standard` calendar.

Time coordinates with no annual cycle and explicitly defined calendars
are not supported.
