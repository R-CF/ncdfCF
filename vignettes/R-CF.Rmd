...
Each of the building blocks may have other objects associated with it, such as axes for `CFVariable`. Package `ncdfCF` is built using the `R6` OOP framework and uses class hierarchies throughout. `CFVariable` and `CFArray`, for instance, derive from the common "virtual" ancestor `CFVariableBase` (as does class `CFVerticalParametricTerm` which you will encounter when using *formula terms* for a parametric vertical axis). The crucial but humble "virtual" class `CFAxis` is not used directly, instead you'll work with its seven descendant classes.
